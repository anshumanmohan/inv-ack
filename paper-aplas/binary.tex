...
Therefore within $\lfloor \log_2 (n - a) \rfloor + 1$ applications of $f$, the result will become equal to or less than $a$, so we can choose this number as the budget for \li{bin\_cdn\_wkr} to successfully reach the countdown value before terminating. 
...
\subsection{Inverse Ackermann in $\bigO\left(\log_2 n\right)$ time}
...
of each component of \li{bin_inv_ack} shows that it is linear up to the 
size of input representation.
\begin{equation*}
\runtime_\alpha(n) \ =
\begin{aligned}
& \runtime_{\alpha_3}(n)
+ \sum_{k = 3}^{\alpha(n) - 1} \runtime_{\cdt{\alpha_k}{1}\ }(\alpha_k(n))
