...
The binary system represents~$n$ in $b = \lfloor \log_{2} n \rfloor + 1$ bits.
In both systems, addition and subtraction of two $b$-bit 
numbers is $\Theta(b)$, 
while multiplication is $\Theta \big(b^2\big)$. 
...
As before, we present an intuitive sketch here and put full proofs in 
...
The key is to ignore these functions entirely and focus on functions that can bring their arguments below a threshold via repeated application in 
\emph{logarithmic} time. This allows a log-sized budget for 
\emph{countdown worker}. Simply shrinking by $1$ is no longer good enough;
we need to halve the argument on every application:
...
Therefore within $\lfloor \log_2 (n - a) \rfloor + 1$ applications of $f$, the result will become equal to or less than $a$, so we can choose this number as the budget for \emph{countdown worker} to successfully reach the countdown value before terminating. 
Note that this budget is simply the length of the binary representation 
of \li{n - a}, which we calculate using our function 
\href{example.com}{\li{nat\_size}}.
We have the following Coq definition of countdown on \li{N}:
...
\subsection{An inverse Ackermann computation in $\Theta\left(\log_2 n\right)$ time}
...
first three levels and recursively building higher levels with \emph{countdown}. 
We focus on the inverse Ackermann hierarchy in this paper, 
but the hyperoperations' definitions can also be found in our codebase, 
along with their proofs of correctness. Furthermore, 
...
Next, we define \emph{inverse Ackermann worker} and the 
\emph{inverse Ackermann} in a style nearly identical to that in 
Section~\ref{subsec: inv_ack_hier}. We simply translate to 
\li{N} as described earlier and keep the budget in \li{nat} so it 
can serve as Coq's decreasing argument. The Ackermann function 
has an extra hardcoded level.
\begin{lstlisting}
