...
3 & \text{exponentiation} & a^b & 2^{b + 3} - 3 & \text{logarithm} & \left\lceil \mathsf{log}_a ~ b \right\rceil & \left\lceil \mathsf{log}_2 ~ (b + 3)\right\rceil - 3 \\
[2pt]
4 & \text{tetration} & \underbrace{a^{.^{.^{.^a}}}}_b & \underbrace{2^{.^{.^{.^2}}}}_{b+3} - 3 & \begin{array}{@{}c@{}}\text{iterated}\\[-3pt]\text{logarithm}\end{array} & \left\lceil \mathsf{log}^*_a ~ b \right\rceil & \left\lceil \mathsf{log}^*_2 ~ (b + 3)\right\rceil - 3
\end{array}
\end{centermath}
...
Finally, our techniques are compact: consider the 14 lines of code that invert the diagonal Ackermann function in \li{nat} on the 18th page of this paper (\S\ref{apx:code}).  Our Coq development is about 3,000 lines, split between parallel efforts in unary-encoded and binary-encoded numbers.
