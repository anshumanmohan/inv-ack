...
\begin{lem} \label{lem: sub-runtime}
	The standard $\li{leb}(x, y)$ function returns \li{true} if $x\le y$ and \li{false} otherwise. When the inputs are encoded in the unary, \emph{i.e.} Coq type \li{nat}, $\runtime_{\li{leb}}(n, a) = \min\{a, n\} + 1$. In binary, \emph{i.e.} Coq type \li{N}, $\runtime_{\li{leb}}(n, a) = \lfloor \log_{2}\min\{a, n\} \rfloor+~1$. 
...
\end{lem}
\begin{proof}
	By definitions of \li{leb} and \li{succ} on \li{nat} and countdown, we have 
...
The function $\alpha_2 \equiv \lambda n.(n-2)$
