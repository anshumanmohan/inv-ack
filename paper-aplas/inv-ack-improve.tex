...
\begin{lem} \label{lem: inv-ack-hier-runtime}
	...
	\end{equation*}
	\hspace{5em}where $\forall i, n.~C_i(n) \triangleq \alpha_i^{(\alpha_{i+1}(n) + 1)}(n) + 1 \in \{1, 2\}$
\end{lem}
\textbf{This lemma implies} $\mathbf{\runtime_{\alpha_{i+1}}(n) \ge 
\runtime_{\alpha_i}(n)}$. 
Given some index $i$ \emph{s.t.} $\runtime_{\alpha_i}(n) = \Omega\big(n^2\big)$, each function after $\alpha_i$ in the hierarchy will take at least $\Omega\big(n^2\big)$, thus making $\runtime_{\alpha_i} (n) = \Omega\big(n^2\big)$ per Definition~\ref{defn: inv-ack-worker}. In fact, $i = 2$ suffices.
...
We can prove a bound of $O(n)$ for every level.
\hide{
