We sketch the proof that the inverse Ackermann function in 
...
\end{lem}
The next two lemmas establish the general runtime structure of countdown and tighten the bound when operating on \li{nat}.
...
\subsection{A na\"ive $\alpha_i$ on \li{nat} is $\Omega(n^2)$, and $O(n)$ after optimization} \label{sect: hardcode-lvl2}
...
We will need three crucial technical lemmas to prove the $O(n)$ overall bound.
