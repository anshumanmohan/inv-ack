...
it is straightforward to modify our techniques to compute $\alpha(n, n)$.
...
\begin{defn} \label{defn: 2para-inv-ack-worker}
	The two-parameter inverse Ackermann worker is a function $\alpha^{\W}_{2}$: 
	...
	\alpha^{\W}_{2}(f, n, k, b) = \begin{cases}
	0 & \text{if } b = 0 \vee n\le k \\ 1 + \alpha^{\W}_{2}\big(\cdt{f}{1}\circ f , \cdt{f}{1}(n), k, b-1\big) & \text{if } b \ge 1 \wedge n \ge k+1
	\end{cases}
	\end{equation}
\end{defn}
...
\begin{thm}
	$\displaystyle \alpha(m, n) = 1 + \alpha^{\W}_{2}\left(\alpha_1, \alpha_1\big(\lceil\log_2n \rceil\big), \left\lfloor \frac{m}{n} \right\rfloor, \lceil\log_2n \rceil \right)$.
\end{thm}
\hide{
...
\note[slow contracting has come out of nowhere - \textcolor{magenta}{Please check the rewritten version I wrote below}]{Unfortunately, it is the best we can do if we want to compute countdowns of slow-contracting functions like $\lambda n.(n - 2)$ (which is among both inverse hyperoperations and inverse Ackermann hierarchy!).}
\textcolor{blue}{
