...
Our new Coq definition computes the correct countdown value, but only for strict binary contractions. Fortunately, the inverse hyperoperations $a\angle{n}b$ are all strict binary contractions when $a\ge 2$ and $n \ge 3$, as are the inverse Ackermann hierarchy $\alpha_n$ starting for $n \ge 2$. We focus on the latter in this paper, but the former's definition can also be found in our 
codebase. We can hard-code the initial levels:
\begin{lstlisting}
Fixpoint bin_alpha (m : nat) (x : N) : N :=
  match m with
  | 0%nat => x - 1 | 1%nat => x - 2 | 2%nat => N.div2 (x - 2)
...
\end{lstlisting}
Note that this Coq Fixpoint is a simple translation of 
\li{inv_ack_worker} in \li{nat}, with only the budget still in \li{nat} so it
can serve as the decreasing argument. The inverse Ackermann function can be computed by the following Coq definition:
\begin{lstlisting}
...
\end{lstlisting}
Similar to the inverse Ackermann hierarchy, we hard-code the result for $n = 0,1,2,3$. This ensures the correct output for small $n$, while for larger $n$ we can use \li{inv_ack_worker} with $\alpha_2$, which is the first binary strictly contractive level, as the starting function. Here the budget is \li{nat_size n}, or $\lfloor \log_2n \rfloor + 1$, which is enough for $n\ge 4$. The full proof of correctness can be found in our codebase.
...
\end{aligned}\right)
\end{equation*}
In the first summand, $\forall k.~\runtime\left(\cdt{\alpha_k}{1}, \alpha_k(n)\right) = \runtime\left(\alpha_{k+1}, n\right) - \runtime\left(\alpha_k, n\right)$.
Each $\Tleb$ in the second summand is $\bigO\left(\log_2k\right)$, so the total sum is $\bigO\big(\alpha(n)\log_2\alpha(n)\big)$.
The third summand is $\Theta(\alpha(n))$ since $\Tsucc$ takes amortized constant time. The last four summands amount up to $\Theta(\log_2n)$. Therefore,
\begin{equation*}
\runtime\left(\alpha, n\right) =
...
Picking $C_i = 4^{i-1} - 1$ gives $C_2 > 2$ and $C_{i+1} > 4C_i + 2 \ \forall i$, so \linebreak $\runtime\big(\alpha_i, n\big) \le C_i\log_2n \ \forall i\ge 3$ by induction. 
By~(\ref{eq: inv-ack-runtime-improved}), for $n > 3 = \Ack(1)$, $\runtime(\alpha, n) = \runtime\big(\alpha_{\alpha(n)}, n\big) = \bigO\big(4^{\alpha(n)}\log_2n\big)$. 

This bound can potentially be improved by further tightening the above inequalities. We do not obtain an exact asymptotic runtime as in 
Theorem~\ref{thm: inv-ack-hier-runtime-improved} because the bound $4^{\alpha(n)}\log_2n$ is larger than the lower bound of $\log_2n$. However, it is still very small and can be bounded by a simple expression like $(\log_2n)^2$.
