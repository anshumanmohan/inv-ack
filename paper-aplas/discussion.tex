...
\end{lstlisting}
Note that for all $n > 7$, $n < \Ack\big(\lfloor \log_2n \rfloor + 1\big)$ $= \Ack\big(\li{nat_size}(n)\big)$, thus a budget of $\li{nat_size}(n)$ is sufficient for the worker to reach the correct inverse Ackermann value before terminating. The full proof of correctness for \li{bin_inv_ack} can be found in our Github repository.
