...
\subsection{The Inverse Hyperoperations, including Div, Log, and Log*}
...
\begin{defn} \label{defn: inv-hyperop}
	...
	\end{cases}
	\end{equation}
	We use $a\angle{n{-}1}$ in a Curried style to denote the single-variable function $\lambda b.a\angle{n{-}1}b$.
\begin{lstlisting}
...
We now show that $a\angle{n}$ is the inverse to $a[n]$. First note that $a\angle{0}\in \contract_0$.~Then:
\begin{lem}
$a\angle{1}b = b - a$.
\end{lem}
\begin{proof}
...
$\forall a \ge 2.~ Q(n)$. \\
Note \mbox{$\forall n.~ a\angle{n+1}$} $= \cdt{a\angle{n}}{a_n}$ and $a[n+1] = \rf{a[n]}{a_n}$. By Theorem~\ref{thm: cdt-inv-rf},
...
These are both straightforward. Then, $Q(1)$ holds by~(\ref{eq: tmp-induction-1}). In the second goal, we have $a \ge 1$.
$P(1)$ holds by $P(0)$ and~(\ref{eq: tmp-induction-2}).
Then, $Q(2)$ holds by $Q(1)$ and~(\ref{eq: tmp-induction-1}).
In the third goal, we have $a\ge 2$, and, using~(\ref{eq: tmp-induction-1}) and $Q(0)$, the goal reduces to $P(n)$. Using~(\ref{eq: tmp-induction-2}) and the fact that $\forall n \neq 1.~a_n\ge 1$, the goal reduces to $P(2)$. This is equivalent to:
\begin{equation*}
...
\begin{rem}
Three early hyperoperations are $a[2]b = ab$, $a[3]b = a^b$ and $a[4]b = \! ^ba$, so by Theorem~\ref{thm: inv-hyperop-correct}, we can define their inverses $\left\lceil b/a \right\rceil$, $\left\lceil \log_a b \right\rceil$, and $\log^*_a b$ as $a\angle{2}b$, $a\angle{3}b$, and $a\angle{4}b$.
...
\end{equation}
This is equivalent to $\alpha_{i+1}(n) = \min\big\{m : \big( \alpha_i \big)^{(m+1)}(n)\le 1\big\}$, or $\alpha_{i+1}(n) = \cdt{\alpha_i}{1}\big(\alpha_i(n)\big)$. From Equation \ref{eq: inv-ack-hier-derive} we can thus define the inverse Ackermann hierarchy, given by $\alpha_i$:
...
$
\end{defn}
Crucially, this gives us a new definition of the inverse Ackermann function
that is \emph{independent of $\Ack(i)$}:
...
\begin{proof}
	...
	\begin{enumerate}[leftmargin=*]
		...
		(\ref{eq: inv-ack-worker-recursion}) with $b\ge 1$ and $\alpha_0(n)\le 1$. Since $\alpha_1 = \big(\cdt{\alpha_0}{1}\big) \circ \alpha_0$ per Definition~\ref{defn: inv-ack-hier}, $P(0)$ holds.
		...
		\end{equation*}
		Per Definition~\ref{defn: inv-ack-hier}, $\alpha_{k+2} = \big(\cdt{\alpha_{k+1}}{1}\big)\circ \alpha_{k+1}$, so this 
		is~(\ref{eq: inv-ack-worker-recursion}) with $b - k - 1\ge 1$ and $\alpha_{k+1}(n)\ge k+2$. Thus $P(k+1)$ holds. Induction on $k$ completes the proof. %\vspace*{-\baselineskip}
	\end{enumerate}
	% \vspace{-2.2em}
\end{proof}
\hide{\color{red}
