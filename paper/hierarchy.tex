...
The next hyperoperation is built upon the previous using repeated application. Addition is level 1, and $b$ repetitions of addition
...
For all $a\ge 1$, $F\in \repeatable_a \implies f\triangleq F^{-1}_+\in \contract_a$.
\end{thm}
\begin{proof}
...
Now since $n\ge a+1$, $\cdt{f}{a}(n)\ge 1$ by the above. Let $\cdt{f}{a}(n) = p+1$. It remains to prove $\cdt{f}{a}(f(n))\le p$, or $f^{(p)}(f(n))\le a$. But this is equivalent to $f^{p+1}(n)\le a$, which holds by $p$'s definition.
\end{proof}
...
For any $a\in \mathbb{N}$ and $f: \mathbb{N}\to \mathbb{N}$, the \emph{countdown worker} of $f$ to $a$ is a function $\W\cdt{f}{a}: \mathbb{N}^2\to \mathbb{N}$ such that:
...
\end{lstlisting}
It is clear that with a sufficient budget, \emph{countdown worker} should compute the correct \emph{countdown} value when $f$ is a contraction strict from $a$. We will show that a budget of $n$ is sufficient. Let us use following Coq-compatible definition of \emph{countdown}, which is also used in our code base.
\begin{defn} \label{defn: countdown}
In this section, define the countdown to $a$ of $f$ to be a function such that: $\cdt{f}{a}(n) = \W\cdt{f}{a}(n, n) \ \forall n$.
\begin{lstlisting}
...
The next lemma shows that \emph{countdown worker} returns $0$ when $n\le a$, which is intended for \emph{countdown}. It follows trivially from \cref{defn: countdown-worker}.
...
\begin{proof}
	...
	\begin{enumerate}[leftmargin=*]
		...
		This is in fact $P(0)$ with $(b, n)$ being substituted for $\left(b-i-1, f^{(1+i)}(n)\right)$. Since $f^{(1+i)}(n) \ge a+1$ and $b-i-1\ge 1$, the above holds and the proof is complete.\vspace*{-\baselineskip}
	\end{enumerate}
\end{proof}
...
Using \cref{lem: contract-repeat-threshold}, there exists an $m$ such that
	\begin{align}
	...
	The former follows by contradiction (LEM on $\mathbb{N}$ is implicit here): if $f^{(m-1)}(n) \le a$, \eqref{eq: cdt-repeat-tmp-2} implies $m\le m-1$, which is wrong for $m > 0$. The latter then easily follows by $f\in \contract_{a}$:
	$$ n \ge 1 + f(n) \ge 2 + f(f(n)) \ge \cdots \ge m + f^{(m)}(n) $$
	Therefore, $\cdt{f}{a}(n) = m$ in all cases, which completes the proof.
\end{proof}
Now, \eqref{eq: rf-upp-inv} and \cref{thm: cdt-repeat} are already enough to establish the correctness of the Coq definitions \emph{countdown} and \emph{countdown worker}. We thus unite \cref{defn: informal-countdown,defn: countdown} as they are equivalent. We wrap everything up in the following theorem.
\begin{thm} \label{thm: cdt-inv-rf}
	If $a\ge 1$ and $F\in \repeatable_a$, then $\rf{F}{a}\ \in \repeatable_0$ and $f\triangleq F^{-1}_+$ satisfies $f \in \contract_{a}$ and $\displaystyle \cdt{f}{a} \ = \left(\rf{F}{a}\ \right)^{-1}_+ \in \contract_0$.
\end{thm}
\begin{proof}
	A simple induction shows that $F^{(n)}(a)\ge a + n\ge 1+n \ \forall n$, so $\rf{F}{a}\ \in \repeatable_0$. $f\triangleq F^{-1}_+ \in \contract_{a}$ and $\left(\rf{F}{a}\ \right)^{-1}_+ \in \contract_0$ follows both by \cref{thm: expansion-inv-contraction}. Finally $\cdt{f}{a} \ = \left(\rf{F}{a}\ \right)^{-1}_+$ due to \eqref{eq: rf-upp-inv}.
\end{proof}
