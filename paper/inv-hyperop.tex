...
We now prove that $a\angle{n}$ is the inverse to $a[n]$.  First, note that $\forall a.~ a\angle{0}\in \contract_0$. Then:
\begin{lem}
$a\angle{1}b = b - a$.
\end{lem}
\begin{proof}
...
\end{equation*}
which is trivial for $a\ge 2$. Induction on n gives us the third goal and thus the proof.
\end{proof}
...
\begin{defn} \label{defn: inv-ack-hier}
    Define the inverse Ackermann hierarchy by $
...
\begin{proof}
	...
	\begin{enumerate}[leftmargin=*]
		...
		\end{equation*}
		Per \cref{defn: inv-ack-hier}, $\alpha_{k+2} = \big(\cdt{\alpha_{k+1}}{1}\big)\circ \alpha_{k+1}$, so this is just \eqref{eq: inv-ack-worker-recursion} with $b - k - 1\ge 1$ and $\alpha_{k+1}(n)\ge k+2$, so $P(k+1)$ holds. The proof is complete by induction on $k$.\vspace*{-\baselineskip}
	\end{enumerate}
\end{proof}
\hide{\color{red}
\begin{proof}
