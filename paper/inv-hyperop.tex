In this section, we apply the \emph{countdown} operation to build the inverse hyperoperation hierarchy, along the way we will define division, logarithm and iterated logarithm in terms of \emph{countdown}, before modifying the inverse hyperoperations to sketch an inverse Ackermann hierarchy.
...
\begin{defn} \label{defn: inv-ack-hier}
	The \emph{inverse Ackermann hierarchy} is a series of functions $\{\alpha_i\}_{i=0}^{n}$ such that
	$$ \alpha_i = \begin{cases}
...
\begin{lem} \label{lem: inv-ack-worker-intermediate}
	For all $n, b, k$ such that $k+1\le \min\big\{b, \alpha_k(n)\big\}$, $$\W\alpha\big(\alpha_0, \alpha_0(n), 0, b\big) = \W\alpha\big(\alpha_{k+1}, \alpha_{k+1}(n), k+1, b-k-1\big)$$
\end{lem}
\begin{proof}
	...
	\begin{enumerate}[leftmargin=*]
		\item \emph{Base case}. $ \W\alpha\big(\alpha_0, \alpha_0(n), 0, b\big) = \W\alpha\big(\cdt{\big(\alpha_0}{1}\big)\circ \alpha_0, \cdt{\alpha_0}{1}\big(\alpha_0(n)\big), 1, b - 1\big) $ by $\W\alpha$'s recursive rule with $b\ge 1$ and $\alpha_0(n)\le 1$. Since $\alpha_1 = \big(\cdt{\alpha_0}{1}\big) \circ \alpha_0$ per \cref{defn: inv-ack-hier}, $P(0)$ holds.
		...
		Per \cref{defn: inv-ack-hier}, $\alpha_{k+2} = \big(\cdt{\alpha_{k+1}}{1}\big)\circ \alpha_{k+1}$, so this is just $\W\alpha$'s recursive rule with $b - k - 1\ge 1$ and $\alpha_{k+1}(n)\ge k+2$.
	\end{enumerate}
