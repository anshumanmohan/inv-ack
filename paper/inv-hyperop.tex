...
$\forall a \ge 2.~ Q(n)$. 
Note that $\forall n.~ a\angle{n+1} = \cdt{a\angle{n}}{a_n}$ and $a[n+1] = \rf{a[n]}{a_n}$. By \cref{thm: cdt-inv-rf},
...
\end{equation*}
which is trivial for $a\ge 2$. Induction {\color{red} on blah} gives us the third goal and thus the proof.
\end{proof}
...
However, that appoach is inviable because it still depends on $\Ack_i$. 
Instead, we reexamine the inverse relationship: suppose $\alpha_i = \big(\Ack_i\big)^{-1}_+$ and $\alpha_{i+1} = \big(\Ack_{i+1}\big)^{-1}_+$. Then $\forall m.~\Ack_{i+1}(m) = \big(\Ack_{i}\big)^{(m)} \big(\Ack_{i}(1)\big)$. We then have:
...
\begin{defn} \label{defn: inv-ack-hier}
	The inverse Ackermann hierarchy $\alpha_i$ is defined as:
\begin{equation*}
...
\end{equation*}
\end{defn}
Its correctness has already been established in \eqref{eq: inv-ack-hier-derive}. We can then provide another computation for the inverse Ackermann function:
...
The following theorem formalizes a setting for $\W\alpha$ to work.
\begin{thm} \label{thm: inv-ack-worker-correct}
	$\W\alpha\big(\alpha_0, \alpha_0(n), 0, n\big) = \alpha(n)$.
\end{thm}
We want to show that $\W\alpha$ can correctly compute $\alpha(n)$
given a budget of $n$. 
...
\end{lem}
\begin{proof}
	 We prove $\forall k.~P(k)$ by induction, where
	 \begin{equation*}
	 \begin{aligned}
	 P(k) \quad \triangleq \quad & \alpha_k(n)\ge k+1 \Rightarrow b\ge k+1 \\ & \Rightarrow \W\alpha\big(\alpha_0, \alpha_0(n), 0, b\big) = \W\alpha\big(\alpha_{k+1}, \alpha_{k+1}(n), k+1, b-k-1\big)
	 \end{aligned}
	 \end{equation*}
	\begin{enumerate}[leftmargin=*]
		...
		\end{equation*}
		Per \cref{defn: inv-ack-hier}, $\alpha_{k+2} = \big(\cdt{\alpha_{k+1}}{1}\big)\circ \alpha_{k+1}$, so this is just \eqref{eq: inv-ack-worker-recursion} with $b - k - 1\ge 1$ and $\alpha_{k+1}(n)\ge k+2$, so $P(k+1)$ holds. The proof is complete by induction {\color{red} on blah}.\vspace*{-\baselineskip}
	\end{enumerate}
\end{proof}
{\color{red}
\begin{proof}
