...
\end{lem}
\begin{proof}
	It is easy to show that for all $k$, $\alpha_2^{(k)}(n) = \left\lceil \frac{n+3}{2^k} \right\rceil - 3$. Thus $\runtime\big(\alpha_3, n\big) = $
	\begin{equation*}
\begin{array}{@{}l@{}}
...
This also allows us to improve running time of $\alpha(n)$ per \Cref{defn: inv-ack-hier} by hard-coding the output when $n\le 1 = \Ack(0, 0)$, and starting $\W\alpha$ with $f = \alpha_1$ and $n = \alpha_1(n)$.
\begin{equation*}
