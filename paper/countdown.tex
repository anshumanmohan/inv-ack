...
For reasons that will be clear momentarily, we write $\rf{F}{a}$'s inverse as $\cdt{f}{a}$.  Now
fix $n$ and observe that for all $m$ we have
...
\end{col}
\begin{proof}
	Fix $a$ and $n$. It suffices to prove the $(\!\!\implies\!\!)$ direction, since the other is trivial. Suppose $\cdt{f}{a}(n)\le m$, we get $f^{(m)}(n)\le f^{(\cdt{f}{a}(n))}(n)$ due to $f\in \contract$, and $f^{(\cdt{f}{a}(n))}(n)\le a$ due to \eqref{eq: countdown}, which completes the proof.
\end{proof}
