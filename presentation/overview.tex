...
\uncover<4->{
\imppar{\text{\textbf{Final Goal.} Compute $\alpha(n)$ without computing $\Ack$.}}
}
...
\uncover<2->{
Let's treat the first argument as an ``index'': define $\Ack_n \triangleq \lambda b.A(n,b)$, then for $n>0$, $\Ack_n$ is the result of applying the previous $\Ack_{n-1}$ $b$ times.
}
...
$\Ack_n(b) = 2[n](b+3) - 3$
\pause
, \quad $\alpha_n(b) = 2\angle{n}(b+3) - 3$.
