...
contracting f
-> (k <= l)
-> repeat f l n <= repeat f k n.
Proof.
intros f n k l Hf Hkl. induction l.
- inversion Hkl. omega.
- inversion Hkl.
  + omega.
  + apply IHl in H0. apply (Nat.le_trans _ (repeat f l n) _).
    * apply Hf.
    * apply H0.
Qed.
...
-> S a <= repeat f k n
-> (S k) + repeat f (S k) n <= n.
Proof.
intros a f n k Hf Haf Han.
induction k.
{ simpl in Han. simpl. apply Haf in Han. omega. }
apply (Nat.le_trans _ (S k + repeat f (S k) n) _).
- apply Haf in Han. simpl. simpl in Han. omega.
- assert (S a <= repeat f k n) as Han0.
  { apply (Nat.le_trans _ (repeat f (S k) n) _).
    - apply Han.
...
- destruct H as [H0 H1]. destruct H1 as [Hl Hr]. inversion Hl.
  * exists (S x). simpl. rewrite H1.
    split.
    apply (Nat.le_trans _ (S x + (repeat f (S x) n)) _).
    simpl. rewrite H1. omega.
    apply (repeat_contract_strict a f n x). apply Hf. apply Haf. omega.
    split. apply Ha. omega. omega.
  * exists x. split. apply H0. omega.
Qed.
...
Lemma repeat_incr : forall k f, increasing f -> increasing (repeat f k).
Proof.
unfold increasing. intros k f Hf.
induction k.
{ simpl. trivial. }
intros n m Hnm. simpl.
apply IHk in Hnm.
apply Hf in Hnm.
apply Hnm.
Qed.