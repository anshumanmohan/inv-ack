...
\begin{table}[t]
	\begin{centermath}
		\begin{array}{c | c@{\hspace{1.5em}} c@{\hspace{1.5em}} c@{\hspace{1.5em}} c@{\hspace{1.5em}} c@{\hspace{1.5em}} c@{\hspace{1.5em}} c@{\hspace{1.5em}} c@{\hspace{1.5em}} c@{\hspace{1.5em}} c@{\hspace{1.5em}}}
			...
			\alpha_3 & 0 & 0 & 0 & 0 & 0 & 0 & 1 & 1 & 1\tikzmark{eight_succ} & \tikzmark{nine_fail4}1 \\
		\end{array}
	\end{centermath}
...
\end{table}
  \begin{tikzpicture}[overlay, remember picture, yshift=.25\baselineskip, shorten >=.5pt, shorten <=.5pt]
    ...
    \draw [->, green] ({pic cs:one}) to [bend left]({pic cs:one_succ});
   	...
   	\draw [->, red] ({pic cs:ninea}) to [bend right]({pic cs:nine_fail4});
  \end{tikzpicture}

\vspace{-3em}
We provide intuition for our new definition in Table~\ref{table:inv_intuition}. 
For any $n$, imagine searching down its column for an $\alpha_k(n) \le k$.
This table is populated exactly as suggested by 
Table~\ref{table: hyperop-ack-inv} earlier.
Red arrows represent unsuccessful searches, and green arrows represent 
successful searches.
