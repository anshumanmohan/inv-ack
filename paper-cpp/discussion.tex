...
In Table~\ref{table:benchmarking} we show the results of 
...
are $3$-time averages of wallclock timings. 
Note that we are dealing with large enough numbers that 
factors such as garbage collection begin to present some variance.

\subsection{Implementing our techniques in Isabelle/HOL}

To show that our techniques translate well and 
are free from unstated Coq dependencies, 
we implemented our work in Isabelle/HOL.  Although we
had no previous experinece with Isabelle/HOL, it took us
...
uses a ``hack'' (a \li{[code]} annotation) to generate a computable equivalent for extraction.
...
\noindent Note that this hack requires a pre-developed computational strategy for the \li{[code]} extraction-substitution lemma.  Thus, extracting the inverse Ackermann via this technique would require the functions we have presented in this paper (or their equivalent): there ain't no such thing as a free lunch~\cite{moonmistress}. Further, we found no definitions of $\log^*$ or the inverse Ackermann function in the Isabelle documentation. Our technique gives directly extractable solutions to the entire hierarchy.
