...
The single-variable \emph{diagonal} Ackermann function written $\Ack : \mathbb{N} \to \mathbb{N}$ is defined as $\Ack(n)~\triangleq~A(n, n)$.  
...
\href{https://github.com/inv-ack/inv-ack/blob/7270e64a2600b771f2b1b1b151f7d13fb2ae6c97/repeater.v#L161-L177}{\color{blue}\emph{kludge}}. %Linked by Linh
...
\begin{itemize}
    ...
    \item[\S\ref{sec: inv-ack}] We give a Coq computation for the inverse Ackermann function in $\bigO(n)$ time.
    \item[\S\ref{sec: binary}] We extend our work to calculate inverse Ackermann for binary inputs. We show that this takes time~$\bigO(b)$, where~$b$ is the bitlength of the binary encoding of the input $n$.
    \item[\S\ref{sec: discussion}] We discuss the value of linear time inverses for
    explosively growing functions, and extend our work to calculate the two-argument inverse Ackermann function.
    \item[\S\ref{sec:related}] We present historical notes, survey related work, and finally conclude.
\end{itemize}
