...
`\href{https://github.com/inv-ack/inv-ack/blob/7270e64a2600b771f2b1b1b151f7d13fb2ae6c97/inv_ack_standalone.v#L6-L11}{Fixpoint cdn\_wkr}` a f n b :=
  match b with 0 => 0 | S b' =>
    if (n <=? a) then 0
...
`\href{https://github.com/inv-ack/inv-ack/blob/7270e64a2600b771f2b1b1b151f7d13fb2ae6c97/inv_ack_standalone.v#L32-L38} {Fixpoint inv\_ack\_wkr}` f n k b :=
  match b with 0 => 0 | S b' =>
    if (n <=? k) then k
...
%Open Scope N_scope.
%
%`\href{https://github.com/inv-ack/inv-ack/blob/7270e64a2600b771f2b1b1b151f7d13fb2ae6c97/inv_ack_standalone.v#L56-L64}{Definition nat\_size}` (n : N) : nat :=
%  match n with
%  | 0 => 0%nat
...
%  end.
%
%`\href{https://github.com/inv-ack/inv-ack/blob/7270e64a2600b771f2b1b1b151f7d13fb2ae6c97/inv_ack_standalone.v#L67-L72}{Fixpoint bin\_cdn\_wkr}` (f : N -> N) (a n : N) (b : nat) : N :=
%  match b with
%  | O    => 0
%  | S b' => if (n <=? a) then 0
...
%  end.
%
%`\href{https://github.com/inv-ack/inv-ack/blob/7270e64a2600b771f2b1b1b151f7d13fb2ae6c97/inv_ack_standalone.v#L75-L76}{Definition bin\_countdown\_to}` (f : N -> N) (a n : N) : N := 
%  bin_cdn_wkr f a n (nat_size (n - a)).
%
%`\href{https://github.com/inv-ack/inv-ack/blob/7270e64a2600b771f2b1b1b151f7d13fb2ae6c97/inv_ack_standalone.v#L97-L104}{Fixpoint bin\_inv\_ack\_wkr}` (f : N -> N) (n k : N) (b : nat) : N :=
...
%  end.
%
%`\href{https://github.com/inv-ack/inv-ack/blob/7270e64a2600b771f2b1b1b151f7d13fb2ae6c97/inv_ack_standalone.v#L111-L116}{Definition bin\_inv\_ack}` n :=
...
`\href{https://github.com/inv-ack/inv-ack/blob/7270e64a2600b771f2b1b1b151f7d13fb2ae6c97/inv_ack_standalone.v#L56-L64}{Definition nat\_size}` n :=
  match n with
  | 0 => 0%nat
  | Npos p =>
...
`\href{https://github.com/inv-ack/inv-ack/blob/7270e64a2600b771f2b1b1b151f7d13fb2ae6c97/inv_ack_standalone.v#L67-L72}{Fixpoint bin\_cdn\_wkr}` f a n b :=
  match b with
  | O    => 0
  | S b' => if (n <=? a) then 0
    else 1 + bin_cdn_wkr f a (f n) b'
...
`\href{https://github.com/inv-ack/inv-ack/blob/7270e64a2600b771f2b1b1b151f7d13fb2ae6c97/inv_ack_standalone.v#L75-L76}{Definition bin\_countdown\_to}` f a n := 
bin_cdn_wkr f a n (nat_size (n - a)).

`\href{https://github.com/inv-ack/inv-ack/blob/7270e64a2600b771f2b1b1b151f7d13fb2ae6c97/inv_ack_standalone.v#L97-L104}{Fixpoint bin\_inv\_ack\_wkr}` f n k b :=
  match b with 0%nat => k | S b' => 
    if n <=? k then k else 
      let g := (bin_countdown_to f 1) in
        bin_inv_ack_wkr
...
\begin{usethmcounterof}{thm: cdt-repeat}
	\href{https://github.com/inv-ack/inv-ack/blob/7270e64a2600b771f2b1b1b151f7d13fb2ae6c97/countdown.v#L162-L189}{\coq}
...
Now we are ready to prove the correctness of the \emph{countdown} computation. \begin{proof}[Proof of Theorem~\ref{thm: cdt-repeat}]
	Since $f\in \contract_{a}$ and $\mathbb{N}$ is well-ordered, let $m = \min\big\{i : f^{(i)}(n)\le a\big\}$.\footnote{We prove the existence
		...
		\href{https://github.com/inv-ack/inv-ack/blob/7270e64a2600b771f2b1b1b151f7d13fb2ae6c97/countdown.v\#L129-L159}{here}
...
\begin{lem} \label{lem: inv-ack-worker-intermediate}
	\href{https://github.com/inv-ack/inv-ack/blob/7270e64a2600b771f2b1b1b151f7d13fb2ae6c97/inv_ack.v#L172-L195}{\coq}
...
of constructors \li{N0} and \li{Npos}. The latter, \li{Npos}, unfolds directly to \li{positive}:
...
To represent $0$, the type \li{N} simply provides a separate constructor \li{N0}.
