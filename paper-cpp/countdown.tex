...
\begin{enumerate}
	...
	\item $f(n)$ is itself in the set, since plugging in $m \triangleq f(n)$ yields $n\le F(f(n))$, which makes $f$ the upper inverse of $F$. \\ \vspace{-1.2em}
\end{enumerate}	
...
$(F^{-1} \circ F)(n) \le n$.  Next, by ($\Rightarrow$) of the same theorem, $(F^{-1} \circ F)(n) \le (F^{-1} \circ F)(n)$ implies that $F(n) \le F \big((F^{-1} \circ F)(n)\big)$. Now $F$ is strictly increasing, so \lb we get $n~\le~(F^{-1} \circ F)(n)$.
\end{proof}
