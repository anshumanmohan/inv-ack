...
However, it only makes sense as an ``inverse'' \lb if~$F$ is strictly
increasing, \emph{i.e.} $\forall n,m.~n < m \Rightarrow F(n) < F(m)$, which is
a rough analogue of injectivity in the discrete \lb domain.
...
\end{thm}
\begin{proof}
Fix $n$. The sentence $\forall m.~ f(n)\le m \iff n\le F(m)$ \lb implies: (1) $f(n)$ is a lower bound to $\{m: n \le F(m)\}$ and \lb (2) $f(n)$ is itself in the set, since plugging in $m \triangleq f(n)$ yields $n\le F(f(n))$, which makes $f$ the upper inverse of $F$. \lb Conversely, if $f$ is the upper inverse of $F$, we know that \lb $\forall m.~n\le F(m)\Rightarrow f(n)\le m$. Finally, by increasing-ness, $\forall m \ge f(n)$.~$F(m) \ge F(f(n)) \ge n$. 
\end{proof}
...
Then $\rf{F}{a}$ 
\href{https://github.com/inv-ack/inv-ack/blob/7270e64a2600b771f2b1b1b151f7d13fb2ae6c97/increasing_expanding.v#L143-L145}{\color{blue}``preserves'' the two properties} of~$F$. The only difference is that it is 
a strict expansion from $0$ instead of from $a$.
We thus say that such a function $F$ is
\href{https://github.com/inv-ack/inv-ack/blob/7270e64a2600b771f2b1b1b151f7d13fb2ae6c97/increasing_expanding.v#L111-L112}{\color{blue}\emph{repeatable from}} $a$, and denote the
...
This only works if each chainlink is strictly smaller than the previous,
\emph{i.e.} $f$ is a \emph{contraction}:
% Linked by Linh
\begin{defn} \label{defn: contracting}
	A function $f : \mathbb{N} \to \mathbb{N}$ is a
	\href{https://github.com/inv-ack/inv-ack/blob/7270e64a2600b771f2b1b1b151f7d13fb2ae6c97/countdown.v#L40-L42}{\color{blue}\emph{contraction}} if $\forall n.~ f(n) \le n$. Given an $a \ge 1$, a contraction $f$ is
...
\end{defn}
% \noindent What kinds of functions have contractive inverses? Expansions, naturally:
...
If $a \in \mathbb{N}$ and $f\in \contract_{a}$, then \lb $\forall n, m.~ \cdt{f}{a}(n)\le m \iff f^{(m)}(n)\le a$.
\end{col}
\begin{proof}
...
\end{thm}
\begin{proof}
When $n \le a$, use Corollary~\ref{col: cdt-repeat} as follows: \lb
...
\linebreak f^{(p)}(f(n))\le a \Rightarrow f^{(p+1)}(n)\le a$, which holds by 
...
If the input goes below or equals $a$ after $k$ applications, \lb \emph{i.e.} $f^{(k)}(n)~\le~a$, we return the count $k$. If the budget is exhausted (\emph{i.e.} $b = 0$) while the result is still above $a$, we fail by returning the original budget. This definition is admittedly inelegant, but the point is that it can clearly be written as a Coq Fixpoint:
% Linked by A
%\begin{lstlisting}
...
we always accompany its use with a constant-time calculation for it. 
...
%\hide{
	...
	$\cdw{f}{a}(n, b)$ be the $0^{\text{th}}$ call. A straightforward induction on $i$ shows that in the~$i^{\text{th}}$ call, the arguments will be $\big(f^{(i)}(n), b-i\big)$, while an accumulated amount~$i$ will have been added to the final result. The only exception to this rule will be the last call, which 
...
of the min in Coq’s intuitionistic logic \href{https://github.com/inv-ack/inv-ack/blob/6099297c6ab0e16d14b037fb5ed600c4d22818f6/countdown.v\#L125-L150}{\color{blue}here} in our codebase.}, the case $m = 0$ is trivial. If $m > 0$, substitute $i = m - 1$ in the above equation, we have:
	\begin{equation*}
